/*!
 * @packet brooder.view.photo;
 * @require brooder.util.canvas;
 * @require brooder.util.file;
 * @css brooder.style.photocutter;
 */
Module({
    name: "photocutter",
    ...
    init: function (option) {
        var ths = this;
        var canvas = require("brooder.util.canvas");
    ...
    setImage: function (files) {
        ...
        if (files.length > 0) {
            var file = require("brooder.util.file");
            var filex = file.set(files[0]);
            filex.getImage(function () {
                var ximage = this, _w = ximage.width, _h = ximage.height;
