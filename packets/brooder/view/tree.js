/*!
 * @packet brooder.view.tree;
 * @css brooder.style.treelite;
 */
$.Module({
    name: "treelite",
    extend: "view",
        ...
        dom.find(".arrow").each(function () {
            $(this).click(function () {
                if (ths.option.async && $(this).parent().parent().children(1).children().length === 0) {
                    ...
                    var thss = $(this);
                    ths.getRemoteData(ths.cache[parseInt($(this).parent().parent().attr("nodeid"))].id, function (data) {
                        if (data || data.length) {
