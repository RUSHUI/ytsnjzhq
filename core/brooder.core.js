(function () {
    ...
    brooder.json = json, brooder.is = is, brooder.browser = browser, brooder.prefix = prefix;
                    ...
                    if ((status >= 200 && status < 300) || status === 304) {
                        ...
                        if (this.realType === "json") {
                            ...
                            try {
                                ...
                                throw Error("[rocui] ajax unvaliable json string,url is '" + option.url+"'");
                            }
                        }
    ...
    plugin.prototype.delegateFind = function () {
        ...
        this.dom.find("[data-find]").each(function () {
            ...
            try {
                ...
                console.error("[rocui] plugin finder called error");
            }
        });
    };
                    ...
                    if (re.length === ths.info.length) {
                        ...
                        for (var i = 0; i < ths.info.length; i++) {
                            ...
                            try {
                                ...
                                (new Function("info", "$", "Module", "Option", "module", "require", "domstr", "T", "Plugin", xcode))(d, brooder, brooder.Module, brooder.Option, requiremapping[d.packet],
                                        function (packetName) {
    ...
    packet.getPacketInfo = function (str) {
        var a = str.match(packet.isNote), basepath = this.option.basepath, n = {"_packets_": {}, "packet": "", require: [], include: [], css: [], dom: [], depends: [], domp: []};
        if (a && a.length > 0) {
            var b = a[0];
            b.match(packet.isInfo).forEach(function (a) {
                ...
                if (d.length >= 2) {
                    ...
                    if (key === "require" || key === "include" || key === "dom") {
                        var t = value.split(":");
                        if (t.length > 1) {
                            if (n._packets_[t[1]]) {
                ...
                option.get(optionName, function (ops) {
                    ...
                    if (ths.layout && ths.layout !== "") {
                        ...
                        if (brooder.is.isString(str)) {
                            try {
                                var code = brooder.template(str).code();
